...
const LightPage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all(IMAGES.map((image) => loadImage(image))).catch((err) =>
      console.log("Failed to load something", err)
    );
  }, []);
  ...
  const confirmLoading = () => {
    increaseLoadingState((prevState) => {
      return prevState + 1;
    });
  };
  ...
  return (
    <>
      ...
      >
        <Stage
          IMAGES={IMAGESDATA}
          playOpenFirst={playOpenFirst}
          confirmLoading={confirmLoading}
          doors={doors}
          currentState={currentState}
          reOpenDoors={reOpenDoors}
        />
        <Shadow />
        ...
        {currentState === 1 ? (
          ...
          <Contact IMAGES={IMAGESDATA} />
        ) : currentState === 3 ? (
          <Resume />
        ) : currentState === 4 ? (
          <ProjectLaptop showInModal={showInModal} IMAGES={IMAGESDATA} />
        ) : (
          ""
        )}
      </div>
